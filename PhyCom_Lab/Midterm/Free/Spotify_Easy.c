...
int main() {
  ...
  if (listen_time > total_play_time) listen_time %= total_play_time;
  int last_queue_listen;
  int idx = 0;
  ...
  if (listen_time == 0) {
    ...
    while (idx < queues && listen_time > song_duration[idx]) {
  ...
  int final_queue = (last_queue_listen == 0 && listen_time > 0) ? 1 : last_queue_listen;

  printf("Song Order: %d\nSong Name: %s\n", idx + 1, queue_song[idx]);
  if (final_queue == 100) printf("Song Process: Complete"); 
  else printf("Song Process: %d%%", final_queue);
  printf("\n");
